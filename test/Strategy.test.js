...
var YandexTokenStrategy = require('../');
var fakeProfile = JSON.stringify(require('./fixtures/profile.json'));

describe('YandexTokenStrategy:init', function () {
  it('Should properly export Strategy constructor', function () {
    assert.equal(typeof YandexTokenStrategy, 'function');
    assert.equal(typeof YandexTokenStrategy.Strategy, 'function');
    assert.equal(YandexTokenStrategy, YandexTokenStrategy.Strategy);
  });

  it('Should properly initialize', function () {
    var strategy = new YandexTokenStrategy({
      clientID: '123',
      clientSecret: '123'
    }, function () {
    });
...
describe('YandexTokenStrategy:authenticate', function () {
  describe('Authenticate without passReqToCallback', function () {
    var strategy;

    before(function () {
      strategy = new YandexTokenStrategy({
        clientID: '123',
        clientSecret: '123'
...
describe('YandexTokenStrategy:userProfile', function () {
